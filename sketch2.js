...
function draw() {
  if(scene < 0){
    ...
    if(frameCount%int(random(100,200))==0) {  
      cx = random(0,width);   
      cy = random(0,-50);
...
function jewelCollect(fishy, Diamond) {
  Diamond.remove();
  collectsound.play();
  score = (score*2);
  console.log(score);  
...
function createDiamond(x,y) { // create powerup
  ...
  // speed intervals 
  newJewel.setSpeed(0.05); 
  newJewel.setVelocity(random(-0.5,0.3), random(0,.5)); 
  // collision box
